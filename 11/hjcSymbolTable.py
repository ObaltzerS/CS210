...
class SymbolTable(object):
    ...
    def __init__(self):
        ...
        self.count = [0, 0, 0, 0] # counts for declarations of kind
    ...
    def Define(self, name, symType, symKind):
        ...
        if name in table:
            message = 'SymbolTable.Define: symbol "{}" already defined'.format(
                          name)
            FatalError(message)
    ...
    def _SelectTable(self, symKind):
        ...
        if symKind in (SYMK_STATIC, SYMK_FIELD):
            ...
            message = 'SymbolTable.Define: unknown symKind ({})'.format(symKind)
            FatalError(message)
